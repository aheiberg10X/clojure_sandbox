(ns highlife.coords (:require [clojure.contrib.math :as math]))
...
(defn coord-in-bounds? [coord dim]
  (let [[x y] coord]
    (if (or (= x dim) (= y dim) (< x 0) (< y 0)) false true)))
...
(defn make-random-coord [dim]
  (vec (take 2 (repeatedly (fn [] (int (math/floor (* dim (rand)))))))))
